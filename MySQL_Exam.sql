create schema SoftUni_Taxi_Company ;
...
CONCAT('(088) 9999',`id`*2)  from drivers as d
...
select dr.first_name, dr.last_name, ca.make, ca.model, ca.mileage
from drivers as dr
left join cars_drivers as cd on dr.id = cd.driver_id
left join cars as ca on cd.car_id = ca.id
where ca.mileage is not null
order by ca.mileage desc, dr.first_name;


select ca.`id`as 'car_id', ca.`make`,ca.`mileage`,
...
select a.name,  if (HOUR(co.start) >= 6 
and hour(co.start) <= 20, 'Day', 'Night') as day_time, 
sum(bill) as bill, cl.full_name, c.make, c.model, cat.name 
as category_name
from addresses a
